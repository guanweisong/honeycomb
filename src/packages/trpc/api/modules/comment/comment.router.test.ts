...
describe('Comment Router', () => {
  ...
  describe('index procedure', () => {
    it('should return comment list', async () => {
      const mockComments = [
        { id: TEST_IDS.ID_1, content: 'Comment 1', status: CommentStatus.PUBLISH, custom: null, page: null, post: null, createdAt: new Date() },
        { id: TEST_IDS.ID_2, content: 'Comment 2', status: CommentStatus.PUBLISH, custom: null, page: null, post: null, createdAt: new Date() },
      ]
  ...
  describe('listByRef procedure', () => {
    it('should return tree structure comments for category', async () => {
      const mockComments = [
        { id: TEST_IDS.ID_1, content: 'Comment 1', status: 'PUBLISH', email: 'test1@example.com', postId: TEST_IDS.ID_1, parentId: null, createdAt: new Date() },
        { id: TEST_IDS.ID_2, content: 'Comment 2', status: 'PUBLISH', email: 'test2@example.com', postId: TEST_IDS.ID_1, parentId: TEST_IDS.ID_1, createdAt: new Date() },
      ]
      const mockCount = [{ count: '2' }]
      ...
      const result = await caller.listByRef({ id: TEST_IDS.ID_1, type: MenuType.CATEGORY })
  ...
  describe('create procedure', () => {
    it('should create comment with captcha validation', async () => {
      const newComment = { id: TEST_IDS.ID_1, content: 'New Comment', status: CommentStatus.PUBLISH }
      ...
      mockDb.select.mockReturnValueOnce(mockDb)
      mockDb.from.mockResolvedValueOnce([{ id: '1', key: 'comment_notify', value: 'true' }] as any)
  ...
  describe('update procedure', () => {
    it('should update comment with admin permissions', async () => {
      const updatedComment = { id: TEST_IDS.ID_1, content: 'Updated Comment', status: CommentStatus.PUBLISH }

      mockDb.update.mockReturnValueOnce(mockDb)
      mockDb.set.mockReturnValueOnce(mockDb)
      mockDb.where.mockReturnValueOnce(mockDb)
      mockDb.returning.mockResolvedValueOnce([{ ...updatedComment, id: TEST_IDS.ID_1 }] as any)
