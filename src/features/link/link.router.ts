...
export const linkRouter = createTRPCRouter({
  ...
  create: permissionProcedure(Permission.linkCreate)
    ...
    .mutation(({ input, ctx }) => createLink(createLinkRepository(ctx.db), input)),
  destroy: permissionProcedure(Permission.linkDelete)
    .input(DeleteBatchSchema)
    .mutation(({ input, ctx }) => destroyLinks(createLinkRepository(ctx.db), input.ids)),
  update: permissionProcedure(Permission.linkUpdate)
    .input(LinkUpdateSchema)
    .mutation(({ input, ctx }) => updateLink(createLinkRepository(ctx.db), input)),
});
