...
export function createPostCommandRepository(db: Database): PostCommandRepository {
  return {
    ...
    async findStatus(id) { const [post] = await observeDbOperation("post.update", "select", () => db.select({ status: schema.post.status }).from(schema.post).where(eq(schema.post.id, id)).limit(1)); return (post?.status as PostStatus | undefined) ?? null; },
