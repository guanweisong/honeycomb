import "server-only";
import type { PostCommandRepository, PostCreateCommand, PostUpdateCommand } from "./repository";
...
/** 创建文章。 */
export function createPost(repository: Pick<PostCommandRepository, "create">, input: PostCreateCommand, authorId: string) { return repository.create(input, authorId); }
/** 批量删除文章。 */
export function destroyPosts(repository: Pick<PostCommandRepository, "destroy">, ids: string[]) { return repository.destroy(ids); }
/** 更新文章。 */
export function updatePost(repository: Pick<PostCommandRepository, "findStatus" | "update">, input: PostUpdateCommand, bus?: InProcessEventBus) { return updatePostThroughAggregate(repository, input, bus); }
/** 替换文章标签关联。 */
