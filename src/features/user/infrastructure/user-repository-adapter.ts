...
export function toUserCommandPort(repository: UserRepository): UserCommandPort {
  return { create: (input) => repository.create(input), getStatus: (id) => repository.getStatus(id), update: (input) => repository.update(input), destroy: (ids) => repository.destroy(ids) };
}
