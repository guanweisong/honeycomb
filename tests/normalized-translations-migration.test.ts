...
function migrationSql(index: string): string {
  const file = readdirSync("drizzle").find((name) => name.startsWith(`${index}_`) && name.endsWith(".sql"));
...
describe("normalized translation migration", () => {
  ...
  beforeEach(async () => {
    directory = mkdtempSync(join(tmpdir(), `honeycomb-translation-${process.getuid?.() ?? "user"}-`));
  ...
  it("backfills exact localized values and removes legacy columns", async () => {
    ...
    const before = Object.fromEntries(
      await Promise.all(
        Object.keys(legacyMultilingualColumns).map(async (table) => [table, await count(client, table)]),
      ),
    );

    await client.executeMultiple(migrationSql("0002"));

    for (const [table, removedColumns] of Object.entries(legacyMultilingualColumns)) {
      expect(await count(client, table)).toBe(before[table]);
      expect(await columns(client, table)).not.toEqual(expect.arrayContaining([...removedColumns]));
    }
    ...
    expect(await client.execute("select locale, content, excerpt from post_translation order by locale")).toMatchObject({
      rows: [
        { locale: "en", content: null, excerpt: "Excerpt" },
        { locale: "zh", content: "正文", excerpt: null },
      ],
    });
    expect(await client.execute("select locale, site_name from setting_translation order by locale")).toMatchObject({
      rows: [
  ...
  it.each([
    ["category", "insert into category (id, description, title, path) values ('bad', '{\"zh\":\"描述\"}', '{\"zh\":\"标题\"}', 'bad')"],
    ["page", "insert into page (id, author_id, content, title) values ('bad', 'author', '{invalid', '{\"en\":\"Title\",\"zh\":\"标题\"}')"],
    ["tag", "insert into tag (id, name) values ('bad', '{\"en\":\"Tag\"}')"],
  ])("rejects invalid required %s translations before dropping old columns", async (table, insertSql) => {
    await client.execute({
      sql: "insert into user (id, email) values (?, ?)",
      args: ["author", "author@example.com"],
    });
    ...
    await expect(client.executeMultiple(migrationSql("0002"))).rejects.toThrow(new RegExp(`${table} translations`, "i"));
    expect(await columns(client, table)).toEqual(
      expect.arrayContaining([...legacyMultilingualColumns[table as keyof typeof legacyMultilingualColumns]]),
    );
  });
  ...
  it.each([
  ...
  ])("rejects invalid optional %s JSON with a diagnostic guard", async (table, insertSql) => {
    const complete = JSON.stringify({ en: "English", zh: "中文" });
    await client.execute({ sql: "insert into user (id, email) values (?, ?)", args: ["author", "author@example.com"] });
    await client.execute({
      sql: "insert into category (id, description, title, path) values (?, ?, ?, ?)",
      args: ["category", complete, complete, "category"],
    });
    ...
    await expect(client.executeMultiple(migrationSql("0002"))).rejects.toThrow(
      new RegExp(`${table} translations`, "i"),
    );
  });
  ...
  it("enforces locale checks and cascade deletion after migration", async () => {
    ...
    await expect(
      client.execute("insert into tag_translation (tag_id, locale, name) values ('tag', 'fr', 'Tag')"),
