...
- 架构重构和大文件拆分默认必须保持现有行为不变；除非用户明确批准，不得借重构顺便修改 API、路由、UI 交互、权限规则、数据模型或错误语义。
- 重构前先锁定现有行为和导出契约，优先补充能够证明边界的失败测试；迁移后必须验证原入口、调用方式和客户端边界仍然成立。
...
- 拆分必须按职责边界进行；如果新文件只是降低行数的包装层，或没有独立的行为/依赖边界，则不应拆分。
...
7. 每次代码改动后必须检查对应测试用例是否需要新增、修改或删除；同时检查 README、测试说明、OpenSpec 任务/设计和 CI 配置等相关文档是否需要同步更新。不能只因现有测试通过就跳过这两项审计；若确认无需改动，必须在交付说明中明确记录检查结论。
