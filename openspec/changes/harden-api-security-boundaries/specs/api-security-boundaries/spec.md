...
### Requirement: 标签列表区分公开与后台可见性
系统 SHALL 提供公开标签列表和受保护的后台标签列表，且公开列表 MUST 仅返回 `ENABLE` 状态的标签。

#### Scenario: 公开查询尝试指定禁用状态
- **WHEN** 未认证调用者通过公开标签接口请求禁用标签
- **THEN** 服务端忽略该状态并仅查询 `ENABLE` 标签

#### Scenario: 后台查询任意状态
- **WHEN** `ADMIN` 或 `EDITOR` 调用后台标签接口并指定状态
- **THEN** 系统按该状态查询标签

#### Scenario: 未认证访问后台标签列表
- **WHEN** 未认证调用者调用后台标签接口
...
系统 SHALL 在读取或创建评论前验证目标资源存在、已发布且启用评论。

#### Scenario: 查询未发布资源的评论
- **WHEN** 调用者查询未发布文章或页面的评论
- **THEN** 系统返回 `NOT_FOUND` 且不查询评论列表

#### Scenario: 对关闭评论的资源提交评论
- **WHEN** 调用者向已发布但关闭评论的资源提交评论
